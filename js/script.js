...
function openFormProfile() {
  //коллбэк для открытия формы профиля
...
function closeFormProfile() {
  //коллбэк для закрытия формы профиля
  formPopupProfile.classList.toggle('popup_is-opened')
}

function submitFormProfile(event) {
  //коллбэк для сабмита формы профиля
  ...
  if (
    ...
    //кнопка "выключена", т.е. данные в форме невалидные
    return
  }
...
function toggleBigSizeImage() {
  //показать/спрятать попап с большой картинкой
...
const cardArr = []
initialCards.forEach(item => cardArr.push(new Card(item)))
